...
import { z } from 'zod'
import { db } from '@/db'
import { contentTags, creatorContentTags, creatorFormats, creatorProfiles, formatTags } from '@/db/schema'
...
export async function saveTags(data: unknown): Promise<TagsActionResult> {
  ...
  const tagRows = allTagCodes.length > 0
    ? await db.select({ id: contentTags.id }).from(contentTags).where(inArray(contentTags.code, allTagCodes))
    : []
